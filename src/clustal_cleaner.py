...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="version",
        help="Print version of Multi-Alignment Cleaner",
    )
    parser.add_argument(
        "-i",
        action="store",
        dest="input",
        help="Multi-alignment file (.aln for CLUSTAL)",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="stars",
        help="Print stars when nucleotides are identical on column",
    )
    parser.add_argument(
        "--consensus",
        action="store",
        dest="consensus",
        help="Print a consensus sequence (in percent)",
    )
...
def read_seq_from_clustal(args):
    ...
    if args.group:
        ...
        for seq_id in group:
            if group[seq_id] > 1:
                sequences[f"group #{count} ({group[seq_id]} seq.)"] = str(
                    sequences[seq_id]
                )
                del sequences[seq_id]

                group_sequences[f"group #{count} ({group[seq_id]} seq.)"] = (
                    group_sequences[seq_id]
                )
        ...
        try:
            with open(
                Path(args.input).parent / Path(args.input).with_suffix(".groups.txt"),
                "w",
            ) as f_out:
            ...
            print(
                "\nWarning: Impossible to create the groups file\n",
                end="",
                file=sys.stderr,
            )
...
def align_sub_sequences(args, sequences):
    ...
    with open(args.sequence_file, "r") as handle:
        ...
        for record in SeqIO.parse(handle, "fasta"):
            ...
            print(
                f"Sequence to position {seq_id} {seq2position[seq_id]} ->  ",
                end="",
                file=sys.stderr,
            )
            # search sequence
        ...
        if args.sequence_file:
            ...
            for idx, seq_id in enumerate(seq_idx):
                ...
                output = output.replace(
                    str(count) * len(target[seq_id]),
                    color + target[seq_id] + SPAN_CLOSE,
                )
