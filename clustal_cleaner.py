...
def align_sub_sequences(args, sequences):
    ...
    seq2position = {}
    seq_idx: dict = {}  # position of primers and probes
    polarity = {}
    ...
    for record in SeqIO.parse(handle, "fasta"):
        ...
        for id in sequences:
            # print(f"{sequences[id]=}", file=sys.stderr)
...
def clean_sequences(args, seq_idx, sequences, seq2position, ref_seq, ref_id, consensus, stars, max_len_id, polarity):
...
def main():
    ...
    SEQUENCES = clean_sequences(
        args, seq_idx, sequences, seq2position, ref_seq, ref_id, consensus, stars, max_len_id, polarity
    )
